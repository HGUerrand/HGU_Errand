...
<div class="container">
        ...
        <div class="actions">
            <div class="actions">
                ...
                <c:if test="${loginMember.role eq 'ADMIN'}">
                    <a href="${pageContext.request.contextPath}/admin/members" class="btn">
                        회원관리
                    </a>
                </c:if>
            </div>
    ...
    <div class="filters">
        ...
        <a href="?category=high&q=${fn:escapeXml(param.q)}"
           class="chip ${currentCategory=='high'?'active':''}">5,000+</a>
    ...
    <div class="errand-grid">
        <c:forEach var="e" items="${errands}">
            <a class="card
                ${e.status eq '완료' ? 'doneCard' : ''}
                ${e.status eq '예약' ? 'reservedCard' : ''}"
